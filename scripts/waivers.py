...
def check(waivers):
    ...
    for i, w in enumerate(waivers):
        ...
        if w["tool"] == "gitleaks" and not w.get("pattern"):
            errors.append(f"{label}: gitleaks waivers need a commit-independent pattern (regex)")
...
def generate(waivers):
    ...
    # gitleaks git-mode fingerprints embed the commit hash and change on every
    # rebase, so waive by a commit-independent regex allowlist instead.
    gl_patterns = [w["pattern"] for w in active if w["tool"] == "gitleaks" and w.get("pattern")]
    semgrep = [w["id"] for w in active if w["tool"] == "semgrep"]
    Path(".trivyignore").write_text("\n".join(trivy) + "\n")
    regexes = ",\n    ".join(f"'''{p}'''" for p in gl_patterns)
    Path(".gitleaks.toml").write_text(
        "[extend]\nuseDefault = true\n\n[allowlist]\n"
        'description = "Waived findings — see waivers/waivers.yaml (CODEOWNERS-gated)"\n'
        'regexTarget = "line"\n'
        f"regexes = [\n    {regexes}\n]\n"
    )
